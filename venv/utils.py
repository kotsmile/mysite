...
import suggest_tool.models as models
import random

BREAKFASTS_PATH = 'suggest_tool/files/recipesold/breakfast.pck'
LUNCHS_PATH = 'suggest_tool/files/recipesold/lunchs.pck'
DINNERS_PATH = 'suggest_tool/files/recipesold/dinners.pck'
SNACKES_PATH = 'suggest_tool/files/recipesold/snackes.pck'

b = mtool.load_pck(BREAKFASTS_PATH)
l = mtool.load_pck(LUNCHS_PATH)
d = mtool.load_pck(DINNERS_PATH)
s = mtool.load_pck(SNACKES_PATH)

mtool.set_recipes('breakfast', [])
mtool.set_recipes('lunch', [])
mtool.set_recipes('dinner', [])
mtool.set_recipes('snack', [])
...
sn = s[:amount]
i = 0
for el in bn + ln + dn + sn:
    i += 1
    print(f'{i}/{amount * 4}')
    new_recipe = models.Recipe(link=el.link, recipe_type=el.recipe_type)
    mtool.add_recipe(new_recipe)

mtool.create_code_to_recipe()
